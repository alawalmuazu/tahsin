        ...
        if (!empty($username)) {
            ...
            if ($query->num_rows() > 0) {
                ...
                if (ENVIRONMENT !== 'production') {
                    log_message('error', "DEV MODE - Password Reset URL for {$arrayData['username']}: {$arrayData['reset_url']}");
                    $this->session->set_flashdata('dev_reset_url', $arrayData['reset_url']);
                }
                $emailSent = $this->email_model->sentForgotPassword($arrayData);
