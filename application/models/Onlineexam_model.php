...
class Onlineexam_model extends MY_Model
{
    ...
    public function examList()
    {
        ...
        if (!is_superadmin_loggedin()) {
            ...
            $this->db->or_where('online_exam.branch_id IS NULL', null, false);
            $this->db->group_end();
        }
    ...
    public function examListDT($currency_symbol = '')
    {
        ...
        if (!is_superadmin_loggedin()) {
            ...
            $this->datatables->or_where('online_exam.branch_id IS NULL', null, false);
    ...
    public function questionListDT($postData)
    {
        ...
        if (!is_superadmin_loggedin()) {
            ...
            $this->datatables->or_where('questions.branch_id IS NULL', null, false);
            $this->datatables->group_end();
            if (!empty($showOwnquestion->show_own_question) && $showOwnquestion->show_own_question == 1) {
    ...
    public function getExamDetails($onlineexamID, $status = true)
    {
        ...
        if (!is_superadmin_loggedin()) {
            $sql .= " AND (`online_exam`.`branch_id` = $branchID OR `online_exam`.`branch_id` IS NULL)";
        }
