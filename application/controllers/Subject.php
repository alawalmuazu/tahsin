    ...
    public function save()
    {
        if ($_POST) {
            ...
            if ($this->form_validation->run() !== false) {
                ...
                // Handle Curriculum PDF Upload
                if (isset($_FILES['curriculum_pdf']) && !empty($_FILES['curriculum_pdf']['name'])) {
                    $config['upload_path'] = './uploads/curriculum/';
                    $config['allowed_types'] = 'pdf';
                    $config['max_size'] = '5120'; // 5MB MAX
                    $config['file_name'] = 'curriculum_' . time();
                    
                    $this->load->library('upload', $config);
                    if ($this->upload->do_upload('curriculum_pdf')) {
                        $uploadData = $this->upload->data();
                        $arraySubject['curriculum_pdf'] = $uploadData['file_name'];
                    } else {
                        $error = $this->upload->display_errors('', '');
                        echo json_encode(array('status' => 'fail', 'error' => array('curriculum_pdf' => $error)));
                        exit();
                    }
                }
