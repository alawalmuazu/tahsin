...
class Employee extends Admin_Controller
{
    ...
    public function add()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() !== false) {
                ...
                $empResult = $this->employee_model->save($post);
                $empID = is_array($empResult) ? $empResult['id'] : $empResult;
    ...
    /* employee csv importer — dedicated page */
    public function csv_import()
    {
        ...
        if (isset($_POST['save'])) {
            ...
            if ($this->form_validation->run() !== false) {
                ...
                if ($csv_array) {
                    $columnHeaders = array('Name','Gender','Religion','BloodGroup','DateOfBirth','JoiningDate','Qualification','MobileNo','PresentAddress','PermanentAddress','Email','NextOfKinName','NextOfKinPhone','NextOfKinRelation');
                    $csvData = array();
                    foreach ($csv_array as $row) {
                        ...
                        if (count($checkCSV) <= 0) {
                            if (filter_var($row['Email'], FILTER_VALIDATE_EMAIL)) {
                                ...
                                if ($query->num_rows() > 0) {
                                    $err_msg .= $row['Name'] . " - Import Failed : Email Already Exists.<br>";
                                } else {
                                    // save all employee information in the database
                                ...
                                $err_msg .= $row['Name'] . " - Import Failed : Invalid Email.<br>";
                            }
                        } else {
                            set_alert('error', translate('invalid_csv_file'));
                            redirect(base_url('employee/csv_import'));
                        }
                    }
                    if ($err_msg != null) {
        ...
        $this->data['title'] = translate('multiple_import') . ' - ' . translate('employee');
        $this->data['branch_id'] = $branchID;
        $this->data['sub_page'] = 'employee/csv_import';
        $this->data['main_menu'] = 'employee';
        $this->data['headerelements'] = array(
            'css' => array(
                'vendor/dropify/css/dropify.min.css',
    ...
    public function csv_export()
    {
        ...
        $results = $this->employee_model->getExportList($branchID, $role);
        ...
        // CSV header
        fputcsv($output, array('Name','Gender','Religion','BloodGroup','DateOfBirth','JoiningDate','Qualification','MobileNo','PresentAddress','PermanentAddress','Email','Role','Designation','Department','NextOfKinName','NextOfKinPhone','NextOfKinRelation'));
