...
class Inventory extends Admin_Controller
{
    ...
    protected function product_validation()
    {
        if (is_superadmin_loggedin()) {
            $this->form_validation->set_rules('branch_id', translate('branch'), 'trim');
        }
    ...
    public function unit()
    {
        if (isset($_POST['unit'])) {
            ...
            $this->form_validation->set_rules('unit_name', translate('unit_name'), 'trim|required|callback_unique_unit');
            if ($this->form_validation->run() !== false) {
                $arrayUnit = array(
    ...
    public function unique_unit($name)
    {
        ...
        $uniform_row = $this->db->get('product_unit')->num_rows();
        if ($uniform_row == 0) {
            return true;
        } else {
