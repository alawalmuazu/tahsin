...
class Student extends Admin_Controller
{
    ...
    public function csv_export()
    {
        ...
        $this->db->select('s.first_name, s.last_name, s.blood_group, s.gender, s.birthday, s.mother_tongue, s.religion, s.caste, s.mobileno, s.city, s.state, s.current_address, s.permanent_address, s.category_id, e.roll, s.register_no, s.admission_date, s.email, p.name as guardian_name, p.relation as guardian_relation, p.age as guardian_age, p.father_name, p.mother_name, p.occupation as guardian_occupation, p.mobileno as guardian_mobileno, p.address as guardian_address, p.email as guardian_email');
        ...
        fputcsv($output, array('FirstName','LastName','BloodGroup','Gender','Birthday','MotherTongue','Religion','Caste','Phone','City','State','PresentAddress','PermanentAddress','CategoryID','Roll','RegisterNo','AdmissionDate','StudentEmail','GuardianName','GuardianRelation','GuardianAge','FatherName','MotherName','GuardianOccupation','GuardianMobileNo','GuardianAddress','GuardianEmail'));
                ...
                if ($csv_array) {
                    ...
                    foreach ($csv_array as $row) {
                        ...
                        if (count($csv_chk) <= 0) {
                            ...
                            $r = $this->csvCheckExistsData($row['Roll'], $row['RegisterNo'], $classID, $sectionID, $branchID, $unique_roll);
                            if ($r['status'] == false) {
    ...
    public function csvCheckExistsData($roll = '', $registerno = '', $class_id = '', $section_id = '', $branchID = '', $unique_roll = '')
    {
        $array = array();
        if (!empty($roll)) {
    ...
    {
        // Strip non-digits; skip if empty or too short
        $nin = preg_replace('/[^0-9]/', '', $nin);
        if (empty($nin) || strlen($nin) < 11) {
            return true; // optional field — no conflict
        }
        // Exclude current student when editing
        if ($this->uri->segment(3)) {
            $row = $this->db->select('student_id')->where('id', $this->uri->segment(3))->get('enroll')->row();
            if ($row) {
                $this->db->where_not_in('id', $row->student_id);
            }
        }
        // Check across ALL branches (NIN is national — must be globally unique)
        $this->db->where('nin', $nin);
        $count = $this->db->get('student')->num_rows();
        if ($count === 0) {
            return true;
        }
        // Get conflicting student for a useful error message
        $existing = $this->db->select('first_name, last_name, register_no')
            ->where('nin', $nin)->get('student')->row();
        $name = $existing
            ? "{$existing->first_name} {$existing->last_name} (Reg: {$existing->register_no})"
            : 'another student';
