		...
		if ($getExam['type_id'] != 1) {
			...
			<div style="width: 50%; padding-left: 15px;">
				<table class="table table-condensed table-bordered">
					<tbody>
						<tr>
							<th colspan="4" class="text-center">NERDC Grading Scale</th>
						</tr>
						<tr>
							<th>Grade</th>
							<th>Mark From</th>
					...
					<?php 
					$grade = $this->db->where('branch_id', $getExam['branch_id'])->order_by('lower_mark', 'DESC')->get('grade')->result_array();
					foreach ($grade as $key => $row) {
					?>
						<tr>
							<td style="width: 15%;font-weight:bold"><?=$row['name']?></td>
							<td style="width: 25%;"><?=$row['lower_mark']?></td>
							<td style="width: 25%;"><?=$row['upper_mark']?></td>
							<td style="width: 35%;"><?=$row['remark']?></td>
						</tr>
					<?php } ?>
					</tbody>
				</table>
