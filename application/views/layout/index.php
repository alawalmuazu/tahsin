	...
	<section class="body">
		...
		<div class="inner-wrapper">
			...
			<section role="main" class="content-body">
				...
				if (is_loggedin()) {
				    $_trust_user_id = get_loggedin_user_id();
				    $_trust_role    = $this->session->userdata('loggedin_role_id');
				    $_prev_login = $this->db->select('ip, platform, browser, timestamp')
				                            ->where('user_id', $_trust_user_id)
				                            ->where('role', $_trust_role)
				                            ->order_by('id', 'DESC')
				                            ->limit(1, 1)
				                            ->get('login_log')
				                            ->row();
				    $_trust_time     = isset($_prev_login->timestamp) ? date('d M Y, h:i A', strtotime($_prev_login->timestamp)) : '';
				    $_trust_browser  = isset($_prev_login->browser)   ? $_prev_login->browser   : '';
				    $_trust_platform = isset($_prev_login->platform)  ? $_prev_login->platform  : '';
				    $_trust_ip       = isset($_prev_login->ip)        ? $_prev_login->ip        : '';
				}
				?>
				<?php if (is_loggedin()): ?>
				<div class="session-trust-bar">
				    <div class="trust-inner">
				        <span class="trust-secure"><i class="fas fa-shield-alt"></i> Secure Session</span>
				        <?php if ($_trust_time): ?>
				        <span class="trust-sep">·</span>
				        <span class="trust-detail"><i class="far fa-clock"></i> Last login: <?= $_trust_time ?></span>
				        <?php if ($_trust_browser): ?>
				        <span class="trust-sep hidden-xs">·</span>
				        <span class="trust-detail hidden-xs"><i class="fas fa-globe"></i> <?= html_escape($_trust_browser) ?> / <?= html_escape($_trust_platform) ?></span>
				        <?php endif; ?>
				        <?php if ($_trust_ip): ?>
				        <span class="trust-sep hidden-xs">·</span>
				        <span class="trust-detail hidden-xs"><i class="fas fa-network-wired"></i> <?= html_escape($_trust_ip) ?></span>
				        <?php endif; ?>
				        <?php else: ?>
				        <span class="trust-sep">·</span>
				        <span class="trust-detail"><i class="fas fa-star"></i> First login — welcome!</span>
				        <?php endif; ?>
				    </div>
				</div>
				<?php endif; ?>
