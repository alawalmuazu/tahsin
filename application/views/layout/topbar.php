<header class="header">
	<div class="logo-env">
		...
		$_ctx_term_row = $this->db->select('name')
		                          ...
		                          ->get('exam_term')
		                          ->row();
		$_ctx_term_name = isset($_ctx_term_row->name) ? $_ctx_term_row->name : '';

		// Week number: calculated from Sep 1 of session start year
		// Session label is like "2026-2027" — take first year
		$_ctx_week_num = '';
		if (!empty($_ctx_session_label)) {
		    $_ctx_start_year = (int) substr($_ctx_session_label, 0, 4);
		    // Nigerian academic calendar: Term 1 starts first Monday of September
		    $_ctx_sep1      = mktime(0, 0, 0, 9, 1, $_ctx_start_year);
		    // Roll forward to first Monday of September
		    $_ctx_dow       = (int) date('N', $_ctx_sep1); // 1=Mon, 7=Sun
		    $_ctx_term_start = $_ctx_sep1 + (($_ctx_dow <= 1 ? 0 : (8 - $_ctx_dow)) * 86400);
		    $_ctx_now       = time();
		    if ($_ctx_now >= $_ctx_term_start) {
		        $_ctx_elapsed_days = floor(($_ctx_now - $_ctx_term_start) / 86400);
		        $_ctx_week_num     = (int) floor($_ctx_elapsed_days / 7) + 1;
		        // Cap at 52 weeks to avoid absurd values in long holidays
		        if ($_ctx_week_num > 52) $_ctx_week_num = '';
		    }
		}

		// Today's date
		$_ctx_today = date('D, j M Y');
		?>
			...
			<li>
				...
				<div class="dropdown-menu header-menubox qk-menu">
					<div class="qk-menu-p">
						...
						<?php } if(is_superadmin_loggedin() || get_permission('employee_disable_authentication', 'is_add')) { ?>
							<a href="<?php echo base_url('authentication/dispatch_pending_invites');?>"><i class="fas fa-paper-plane"></i> <?php echo translate('dispatch_invites') ?: 'Dispatch Invites';?></a>
	...
	<?php if (is_loggedin() && ($_ctx_session_label || !empty($_ctx_term_name) || !empty($_ctx_week_num))): ?>
	<div class="ctx-pill hidden-xs hidden-sm">
		<?php if ($_ctx_session_label): ?>
		<span class="ctx-pill-session"><i class="far fa-calendar-alt"></i> <?= html_escape($_ctx_session_label) ?></span>
		<?php endif; ?>
		<?php if (!empty($_ctx_term_name)): ?>
		<span class="ctx-pill-sep">|</span>
		<span class="ctx-pill-term"><?= html_escape($_ctx_term_name) ?></span>
		<?php endif; ?>
		<?php if (!empty($_ctx_week_num)): ?>
		<span class="ctx-pill-week">Wk <?= $_ctx_week_num ?></span>
		<?php endif; ?>
	</div>
	<?php endif; ?>
								...
								<li>
									<a href="<?php echo base_url('communication/mailbox/read?type='.$message['msg_type'].'&id='.$message['id']);?>" class="clearfix">
										...
										<figure class="image pull-right">
											<img src="<?php echo $message['message_details']['imgPath']; ?>" height="40" width="40" class="img-circle" alt="<?php echo html_escape($message['message_details']['userName']); ?>">
										</figure>
...
<div id="offlineStateBanner" class="alert alert-danger" style="display:none; position:fixed; top:0; width:100%; z-index:99999; text-align:center; border-radius:0; margin:0;">
    <i class="fas fa-wifi-slash"></i> <strong>Working Offline</strong> - Data will be saved locally and synced automatically.
</div>
<script>
    window.addEventListener('offline', function() {
        document.getElementById('offlineStateBanner').style.display = 'block';
    });
    window.addEventListener('online', function() {
        document.getElementById('offlineStateBanner').style.display = 'none';
        // swal could also be triggered here if we wanted
    });
    if (!navigator.onLine) {
        document.getElementById('offlineStateBanner').style.display = 'block';
    }
