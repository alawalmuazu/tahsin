<section class="panel">
	...
	<div class="panel-body">
		...
		<?php if (!empty($sections) && !empty($categories)): ?>
			...
			<table class="table table-bordered table-condensed">
				...
				<tbody>
					...
					<tr>
						...
						<?php foreach ($categories as $cat):
							$val = isset($fee_map[$sec->id][$cat->id]) ? $fee_map[$sec->id][$cat->id] : '';
						?>
						<td>
							<input type="number" step="0.01" min="0" class="form-control"
								name="fee[<?php echo (int) $sec->id; ?>][<?php echo (int) $cat->id; ?>]"
		...
		<?php else: ?>
		<div class="alert alert-info">Add Sections and Categories first, then return here to set per-combination fees.</div>
		<?php endif; ?>
	</div>
	<footer class="panel-footer">
		<div class="row">
			<div class="col-md-offset-9 col-md-3">
				<button type="submit" name="save" value="1" class="btn btn-default btn-block" <?php echo !$this->school_fee_model->tableReady() ? 'disabled' : ''; ?>>
