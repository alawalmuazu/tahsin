(function($) {
    ...
    $(document).on('submit', 'form[data-offline-sync="true"]', function(e) {
        if (!navigator.onLine) {
            e.preventDefault();
            e.stopImmediatePropagation();
            var $this = $(this);
            var actionUrl = $this.attr('action');
            var currentFormData = new FormData(this);
            var btn = $this.find('[type="submit"]');
            btn.button('loading');
            
            saveToOfflineQueue(actionUrl, currentFormData).then(() => {
                swal({
                    toast: true,
                    position: 'top-end',
                    type: 'success',
                    title: 'Saved to Offline Queue! It will sync automatically when back online.',
                ...
                alertMsg('Failed to save offline: ' + err, 'error');
                btn.button('reset');
            });
            return false;
        }
